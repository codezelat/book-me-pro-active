...
import { Phone, Mail, CircleX, CircleCheck, CircleChevronDown } from "lucide-react"; 
...
export default function CustomizedDataGrid({ selectedDate, coachId }) {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        const response = await axios.get(`/api/appointments?coachId=${coachId}`);
        ...
        const allAppointments = await response.json(); 
         ...
         const filteredAppointments = allAppointments.filter(appointment => {
          const appointmentSelectedDate = dayjs(appointment.selectedDate).format("YYYY-MM-DD");
          const selectedDateFormatted = dayjs(date).format("YYYY-MM-DD");
          return appointmentSelectedDate === selectedDateFormatted;
        });
  ...
  },
  {
    ...
    renderCell: (params) => (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
        <button style={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "transparent", border: "1", borderColor: "#037D40", padding: "0", cursor: "pointer", marginRight: "8px" }}>
          <Phone size={29} style={{ color: "#037D40", backgroundColor: "#fff", borderRadius: "20%", border: "2px solid #037D40", padding: "5px" }} />
        </button>
        {params.value}
        <button style={{ display: "flex", alignItems: " center", justifyContent: "center", backgroundColor: "transparent", border: "none", padding: "0", cursor: "pointer", marginLeft: "8px" }}>
          <Mail size={29} style={{ color: "#037D40", backgroundColor: "#fff", borderRadius: "20%", border: "2px solid #037D40", padding: "5px" }} />
        </button>
      </div>
    ),
  },
    ...
    renderCell: (params) => (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%" }}>
        <button
          ...
          style={{ display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#037D40", border: "none", padding: "0", cursor: "pointer", marginRight: "8px" }}
        >
          <CircleChevronDown size={30} style={{ color: "#fff", padding: "6px", backgroundColor: "#037D40" }} />
        </button>
      </div>
    ),
  },
