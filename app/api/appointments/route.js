...
async function sendEmail({ email, name, status }) {
  ...
  const BREVO_API_KEY =
    ...
    "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"; // Replace with your Brevo API key or use environment variable
...
export async function GET(req) {
  ...
  try {
    ...
    if (status === "pending") {
      query.selectedDate = { $gte: today }; // Apply date filtering for pending status
    }
...
export async function PATCH(req) {
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      ...
      return new Response(
        JSON.stringify({ message: "Appointment status updated successfully." }),
        { status: 200 }
      );
    } else {
      return new Response(
        JSON.stringify({ message: "Failed to update appointment status." }),
        { status: 500 }
      );
    }
  } catch (error) {
